...
iojs_taken = False
src_domain = "nodejs.org/download/release"
...
def _get_versions_json():
    response = urlopen('https://%s/index.json' % src_domain)
...
def main():
    ...
    if opt.io:
        iojs_taken = True
        src_domain = "iojs.org/download/release"

    if opt.mirror:
        src_domain = opt.mirror
    # use unofficial builds only if musl and no explicitly chosen mirror
    elif is_x86_64_musl():
        src_domain = 'unofficial-builds.nodejs.org/download/release'
